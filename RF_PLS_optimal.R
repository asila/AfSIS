...
required.packages <- c("hexView","caret","readr","mlbench","pROC","rpart","caretEnsemble","soil.spec","FitAR","doParallel","ggplot2","dplyr","downloader","prospectr","randomForest","gridExtra","gridExtra1")
...
calibrate <- function(wd,infrared.data,reference.data,hout,method=c("RF","PLS")){
  ...
  for (q in 1:length(hd)){
  	...
  	# Make prediction using the fitted model and all the spectra in the training data
   ...
   xlab("Measured")+
   ylab("Predicted")
   p1 <- p1+stat_smooth(method=lm, se=FALSE, color='black',alpha=0.1)
   p1 <- p1+theme(plot.title = element_text(lineheight=3, face="bold", color="black", size=20))
   p1 <- p1 + theme(text = element_text(size = 20)) # this will change all text size 
   p1 <- p1 + annotate('text', label=paste('R^2==',RSQ), parse=TRUE,Inf, -Inf,hjust = 2.5, vjust = -7.8) + annotate('text', label=paste('RMSE==',RMSE), parse=TRUE,Inf, -Inf,hjust = 1.8, vjust = -6.4)

  # Save the created models 
  
  ggsave(file=paste0(b,"/","Full_calibration_plots/",hd[q],".png"),p1)
...
# PLS Regression method ----------------------------------------------------------------
  if(method == "PLS"){
  	...
  	#Use Kennard_Stone. This is an optional step just to show distribution of spectra in a PCA space.
	...
	for (q in 1:length(hd)){
		...
		rf.m <- train(trainY~., method="pls", data=cal[,-1],trControl=ctrl,tuneGrid = expand.grid(ncomp = 1:10),metric="RMSE",preProc = c("center", "scale"))
		...
		# Create scatter plot for the predicted versus the measured - validation set
		p2 <- ggplot(pmp, aes(x=measured.test,y=predicted.test))+
		geom_point(col="brown",size=3,alpha=0.2)+
		ggtitle(paste0("Validation for ",hd[q]))+
		xlab("Measured")+
      ...
      for (q in 1:length(hd)) {
      	...
      	pm <- as.data.frame(cbind(y,predi))
      	colnames(pm) <-c ("measured","predicted")
      	png(file=paste0(b,"/","Full_calibration_plots/",hd[q],".png"),height= 600,width=600)
      	p1 <- ggplot(pm, aes(x=measured,y=predicted))+
      	geom_point(col="black",size=5,alpha=0.4)+
      	ggtitle(paste0("Calibration for ",hd[q]))+
      	xlab("Measured")+
      	ylab("Predicted")
      	p1 <- p1+stat_smooth(method=lm, se=FALSE, color='brown',alpha=0.6,size=1.5)+
      	theme(plot.title = element_text(lineheight=3, face="bold", color="black", size=20))+
      	theme(text = element_text(size = 20))+ # this will change all text size 
      	annotate('text', label=paste('R^2==',RSQ), parse=TRUE,Inf, -Inf,hjust = 2.5, vjust = -7.8,size=5) +annotate('text',label=paste('RMSE==',RMSE), parse=TRUE,Inf, -Inf,hjust = 2.0, vjust = -6.8,size=5) + annotate('text', label=paste('PCs==',PCs), parse=TRUE,Inf, -Inf,hjust = 2.5, vjust = -3.9,size=5)
      	
      	# Create scatter plot for the predicted versus the measured - combined dataset
